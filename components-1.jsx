...
function InterestModal({ open, data, onClose }) {
  ...
  if (!data) return null;
  const imgSrcs = data.imgs.map(im => im.src);
  return (
    <>
        ...
        <div className="modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-body">
            ...
            <div className="modal-gallery">
              {data.imgs.map((im, j) => (
                <img
                  key={j} src={im.src} alt={im.label}
                  className="zoomable"
                  onClick={e => { e.stopPropagation(); openLightbox(imgSrcs, j); }}
                />
              ))}
            </div>
...
function Lightbox({ images, startIndex, onClose }) {
  ...
  }, [images, total, onClose]);

  if (!images) return null;
  return (
    <div className={'lightbox-overlay' + (images ? ' open' : '')} onClick={onClose}>
      <button className="lightbox-close" onClick={onClose} aria-label="Fermer">×</button>
      {total > 1 && (
        <button className="lightbox-nav lightbox-prev" onClick={e => { e.stopPropagation(); setIdx(i => (i - 1 + total) % total); }}>←</button>
      )}
      <div className="lightbox-img-wrap" onClick={e => e.stopPropagation()}>
        <img src={images[idx]} alt="" className="lightbox-img" key={idx} />
      </div>
      {total > 1 && (
        <button className="lightbox-nav lightbox-next" onClick={e => { e.stopPropagation(); setIdx(i => (i + 1) % total); }}>→</button>
      )}
      {total > 1 && (
